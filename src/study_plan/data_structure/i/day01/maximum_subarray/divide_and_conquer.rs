...
fn _max_sub_array(nums: &[i32], left_index: usize, right_index: usize) -> i32 {
    ...
    if left_index == right_index {
        ...
        max_3(
            mid_sum(&Vec::from(nums), left_index, mid_index, right_index),
...
fn mid_sum(nums: &[i32], left_index: usize, mid_index: usize, right_index: usize) -> i32 {
    max_sum(nums, (left_index..mid_index).rev(), nums[mid_index])
        + max_sum(nums, mid_index + 1..=right_index, 0)
}

fn max_sum<T>(nums: &[i32], iter: T, init: i32) -> i32
where
